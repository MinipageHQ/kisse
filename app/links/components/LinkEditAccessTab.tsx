...
const settings = [
  ...
  },
  {
    ...
    value: "purchase",
    description: "Anyone who bought any assets from you will be able to access the link ",
  },
  {
...
function List({ settings }) {
  return settings.map((setting, settingIdx) => (
    <RadioGroup.Option key={setting.name} value={setting} as={RadioGroupOption(settingIdx)}>
      {({ active, checked }) => (
        <>
          <span
            className={classnames(
