    ...
    <div>
      ...
      <div className="mt-10 sm:hidden">
        ...
        <ul role="list" className="mt-3 border-t border-gray-200 divide-y divide-gray-100">
          {assets.map((asset) => (
            <li key={asset.id}>
              <Link href={Routes.AssetsPage({ assetQueries: [asset.id] })} passHref>
                <a className="group flex items-center justify-between px-4 py-4 hover:bg-gray-50 sm:px-6">
                  <span className="flex items-center truncate space-x-3">
                    ...
                    <span className="font-medium truncate text-sm leading-6">
                      {asset.name} <span className="truncate font-normal text-gray-500">{asset.assetTypeId}</span>
                    </span>
                  </span>
                  <ChevronRightIcon
                  ...
                  <td className="px-6 py-3 max-w-0 w-full whitespace-nowrap text-sm font-medium text-gray-900">
                    <div className="flex items-center space-x-3 lg:pl-2">
                      ...
                      <a href="#" className="truncate hover:text-gray-600">
                        <span>
                          {asset.name}
                          {asset.description} <span className="text-gray-500 font-normal">in {asset.assetTypeId}</span>
                        </span>
                      </a>
                    </div>
                  </td>
